...
class ErrorCtrl extends BaseCtrl
{
  ...
  public function handleException(Request $request, Response $response, $exception)
  {
    ...
    try {
      $statusCode = 500;
      if ($exception instanceof \App\Exception\NotFound) {
        $statusCode = 404;
      }
      if ($exception instanceof \App\Exception\AccessDenied) {
        $statusCode = 403;
      }

      $data = $this->getData($statusCode);

      if (is_subclass_of($exception, '\App\Exception\GeneralException')) {
